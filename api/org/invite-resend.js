...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!origin || !origin.startsWith('http')) return fail(400, 'INVITE_ORIGIN_INVALID', '邀请来源地址无效')
    ...
    `
    if (!rows.length) return fail(404, 'INVITE_NOT_FOUND', '未找到邀请记录')
    const invite = rows[0]
    ...
    console.error('org/invite-resend', e)
    return fail(500, 'ORG_INVITE_RESEND_FAILED', '重发邀请失败，请稍后重试')
  }
}
