/**
 * 网站衰老评估（登录用户）
 * GET  /api/aging-assessments  各时钟最近一次
 * POST /api/aging-assessments  { kind, input, consentHealthData }
 *
 * 只保存实验室报告解读，不调用营养代谢小程序，也不从原始芯片重跑时钟。
 */
...
import { computeAgingAssessment, isAgingKind, resultToSummary } from '../src/lib/agingClocks/index.js'
...
async function ensureTable() {
  ...
  `
  await sql`CREATE INDEX IF NOT EXISTS idx_aging_assessments_user_kind ON aging_assessments(user_id, kind, created_at DESC)`
  tableReady = true
}
...
export default async function handler(req, res) {
  ...
  if (!user || !canViewContent(user.level, 'standard', { isGuest: false })) {
    return res.status(403).json({ error: '衰老评估仅向标准会员及以上开放' })
  }
  ...
  if (req.method === 'GET') {
    ...
    return res.status(200).json({
      latest,
      agingSummary: summaries.join('；').slice(0, 800),
    })
  }
  ...
  if (req.method === 'POST') {
    ...
    if (!isAgingKind(kind)) return res.status(400).json({ error: '未知衰老评估类型' })

    const computed = computeAgingAssessment(kind, body.input || {})
