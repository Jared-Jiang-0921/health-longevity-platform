...
**收单**：本站会员与咨询升级统一走 **Stripe Checkout**。用户可选银行卡、微信支付、支付宝（需在 Stripe Dashboard 开通对应支付方式）。

**提现**：在 [Stripe Dashboard → 设置 → 银行账户与提现](https://dashboard.stripe.com/settings/payouts) 中配置收款银行账户。
