...
export default async function handler(req, res) {
  ...
  try {
    ...
    const brand = String(process.env.EMAIL_BRAND_NAME || '长健星图 Longevity Atlas').trim()
    const send = await sendPasswordResetEmail({ to: email, resetUrl, brandName: brand })

    if (!send.ok) {
      if (send.reason === 'not_configured') {
        console.warn(
          '[forgot-password] SMTP not configured; set SMTP_HOST, SMTP_USER, SMTP_PASS, and EMAIL_FROM (or rely on SMTP_USER as From). Reset URL (copy for manual testing):',
          resetUrl,
        )
      } else if (send.reason === 'send_failed') {
        console.error('[forgot-password] SMTP send failed:', send.error || send)
      }
    }
